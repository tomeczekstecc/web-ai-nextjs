...
> **Moved.** Variant choice, sizing, icon-only + tooltip, loading state, and the multi-button surface heuristic now live in **`context/button-patterns.md`**. See also `context/destructive-actions.md` for confirm-before-delete flows.
...
> **Moved.** The `resolveIcon` registry, naming rules, sizing convention, and icon a11y patterns now live in **`context/icon-system.md`**.
...
> **Moved.** The full pattern — `AppShell`, `breadcrumbRegistry`, `MenuConfig` fallback, `PageTitle`, sidebar trigger placement, and the per-domain checklist — now lives in **`context/domain-page-layout.md`**.
...
> **Moved.** The intent→variant table, the multi-button heuristic, the worked examples, and the pre-merge checklist now live in **`context/button-patterns.md`** (section "Choosing a Variant at the Call Site").

---

## Destructive Actions Always Confirm

> **Moved.** The full confirm-before-delete pattern — trigger styling, dialog shape, bulk variant, anti-patterns, and the per-action checklist — now lives in **`context/destructive-actions.md`**.

---

## Date Format — ISO by Default

> **Moved.** ISO 8601 rules, `formatDate` / `formatDateTime` / `formatRelative` helpers, the `<RelativeTime>` component, exception table, and full checklist now live in **`context/date-format.md`**.
